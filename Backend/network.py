...
def load_network():
    ...
    for node, data in G.nodes(data=True):
        ...
        # 1. Signalized intersections (from OSM)
        ...
        # 3. Inspect connected road types
        ...
        # 4. Explicit stop/yield tags
        node_highway = data.get("highway")
        traffic_sign = data.get("traffic_sign")
        ...
        if explicit_stop or explicit_yield:
            ...
            has_major = bool(connected_highways & major_roads)
            has_minor = bool(connected_highways & minor_roads)
            data["control"] = "priority" if (has_major and has_minor) else "none"

    # Add speed and travel time data
    G = ox.add_edge_speeds(G)
    G = ox.add_edge_travel_times(G)

    # Apply real traffic data from GeoJSON
